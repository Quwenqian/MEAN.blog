...
angular.module("blog", ["ngAnimate", "ui.router", "angular-loading-bar"]).config(["$stateProvider", "$urlRouterProvider", "cfpLoadingBarProvider", function ($stateProvider, $urlRouterProvider, cfpLoadingBarProvider) {
...
.factory("Markdown", ["$filter", "$state", function ($filter, $state) {
    ...
    var $temp = null;
    var $ret = angular.element("<div></div>");
    ...
    return {
        ...
        summary: function summary() {
            ...
            var len = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 72;

            $temp = parse(md);
            if (plainText) {
                var ret = $filter("ellipsis")($temp.text(), len);
                $temp.empty();
                return ret;
            }
...
.filter("ellipsis", [function () {
    return function (input, chars) {
        var breakOnWord = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
.controller("ListController", ["$rootScope", "$scope", "articles", "Articles", "Markdown", function ($rootScope, $scope, articles, Articles, Markdown) {
    ...
    $scope.remove = function (index) {
        var id = $scope.articles[index]._id;
        swal({
            title: "确认要删除吗？",
            text: "删除后您将无法恢复该文章！",
            type: "warning",
